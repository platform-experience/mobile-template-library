...
branch_checkout() {
  local branch=feature/${PREFIX}${MO}
  git fetch origin
...
make_commit() {
  ...
  cd ../../
  git add ${SRC_DIR}/${PREFIX}${MO}
  git commit -a -m "${COMMIT_MSG} ${mo_name_lower}"
...
set_name() {
  local mo_dir=()
  for i in "${inputs[@]}"; do
    mo_dir+=$(echo -${i} | tr '[:upper:]' '[:lower:]')
    mo_name_upper+=$(make_uppercase ${i})
    mo_name_lower+=$(make_lowercase ${i})
  done
  MO=$(format_data ${mo_dir[@]})
}
...
sub_base_content() {
  ...
  if [[ ${name_has_dashes} ]]; then
    ...
    replace_content "${NAME_TEMP}" "${mo_name_upper%?}" README.md
    replace_content "${NAME_TEMP}" "${mo_name_upper%?}" config.json
  fi
  replace_content "${CONTRIB_TEMP}" "$(fetch_github_user)" config.json
  replace_content "${DIR_TEMP}" "${PREFIX}${MO}" config.json
  replace_content "${DIR_TEMP}" "${PREFIX}${MO}" README.md
}
